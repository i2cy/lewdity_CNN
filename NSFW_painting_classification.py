...
MODEL_FILE = "./Models/NSFW_painting_model.h5"
NAME = "NSFW_painting_detect"
...
def read_preprocess_image(img_path):  # 定义数据集map函数
    try:
        ...
        img = tf.image.resize(img, [265, 265])
        img = tf.image.random_crop(img, [256, 256, 3])
    except Exception as err:
        print("failed to preprocess image \"{}\"".format(img_path))
...
def main():
    ...
    train_img_paths = [str(ele) for ele in data_root.glob("train/painting/*/*.*")]
    test_img_paths = [str(ele) for ele in data_root.glob("test/painting/*/*.*")]
    ...
    if EPOCHES > 0 and choice in ("Y", "y", "yes"):
        try:
            ...
            print("error occured while training:", err)
        trained = True
